...
`npm run check` comprueba pruebas, tipos, compilación y un cliente MCP real desde otro directorio con un ejecutable cuyo nombre contiene espacios y `#`. El envío es simulado. La [plantilla CI](ci/check.yml) prepara Windows y Ubuntu con Node.js 22 y 24, incluyendo normalización, exclusión de un segundo servidor y reinicio del mismo canal. Su activación está pendiente: GitHub rechazó la publicación del workflow por falta de permisos en las credenciales disponibles. Con una credencial que tenga permiso `workflow`, mueva la plantilla a `.github/workflows/check.yml` y publíquela. No hay resultados CI de esta matriz todavía. Las aserciones de permisos POSIX solo se ejecutan en POSIX; la revisión de ACL de la instalación es separada.
